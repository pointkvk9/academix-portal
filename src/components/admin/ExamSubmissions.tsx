...
import { Users, Download, Search, FileText } from "lucide-react";
...
export function ExamSubmissions({ exams, selectedExamId, onSelectExam }: ExamSubmissionsProps) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    // Fetch applications and profiles separately since there's no FK
    const fetchData = async () => {
      ...
      const { data: profiles } = await supabase
        .from("profiles")
        .select("user_id, full_name, email, class, mobile, gender, father_name")
        .in("user_id", userIds);
    ...
    <Card className="border-t-4 border-t-primary">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <Users className="h-5 w-5 text-primary" /> Exam Submissions
        </CardTitle>
      ...
      <CardContent>
        {!selectedExamId ? (
          <div className="text-center py-12">
            ...
            <p className="text-muted-foreground">Select an exam to view submissions</p>
          </div>
        ) : loading ? (
          <div className="text-center py-12">
          ...
          <>
            ...
            <div className="grid grid-cols-4 gap-3 mb-4">
              ...
              <div className="bg-success/10 rounded-md p-3 text-center">
                <p className="text-xl font-bold text-success">{totalSubmitted}</p>
                <p className="text-xs text-muted-foreground">Submitted</p>
              </div>
              <div className="bg-info/10 rounded-md p-3 text-center">
                <p className="text-xl font-bold text-info">{totalPaid}</p>
                <p className="text-xs text-muted-foreground">Fee Paid</p>
              </div>
              <div className="bg-warning/10 rounded-md p-3 text-center">
                <p className="text-xl font-bold text-warning">{totalInProgress}</p>
                        ...
                        <TableCell>
                          <div className="flex items-center gap-1">
                            <div className="w-16 bg-muted rounded-full h-2">
                              <div className="bg-primary rounded-full h-2 transition-all" style={{ width: `${(s.current_step / 6) * 100}%` }} />
                            </div>
                            <span className="text-xs text-muted-foreground">{s.current_step}/6</span>
                          </div>
                        </TableCell>
                        <TableCell>
                          <Badge variant={s.fee_status === "paid" ? "default" : "secondary"} className={s.fee_status === "paid" ? "bg-success text-success-foreground" : ""}>
                            {s.fee_status}
                          </Badge>
                        </TableCell>
                        <TableCell>
