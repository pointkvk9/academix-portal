...
import { LogOut, FileText, Users, MapPin, ClipboardList, LayoutDashboard, BookOpen } from "lucide-react";
          ...
          <TabsContent value="dashboard">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
              {[
                ...
                { label: "Exam Centers", value: stats.totalCenters, icon: MapPin, color: "text-warning", bg: "bg-warning/10" },
              ].map((s, i) => (
                ...
                <CardContent className="p-4">
                  ...
                  {exams.slice(0, 5).map(e => (
                    <div key={e.id} className="flex items-center justify-between py-2 border-b last:border-0">
                      ...
                      <Button size="sm" variant="ghost" onClick={() => { setSelectedExam(e.id); setActiveTab("submissions"); }}>
                        View →
                      </Button>
                    </div>
                  ))}
                ...
                <CardContent className="p-4">
                  ...
                  <div className="space-y-2">
                    <Button className="w-full justify-start" variant="outline" onClick={() => setActiveTab("create")}>
                      <ClipboardList className="h-4 w-4 mr-2" /> Create New Exam
                    </Button>
                    <Button className="w-full justify-start" variant="outline" onClick={() => setActiveTab("centers")}>
                      <MapPin className="h-4 w-4 mr-2" /> Manage Centers
                    </Button>
                    <Button className="w-full justify-start" variant="outline" onClick={() => setActiveTab("submissions")}>
